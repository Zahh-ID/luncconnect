...
const Modal: React.FC<ModalProps> = ({
  ...
  const blockTimeout = useRef<ReturnType<typeof setTimeout>>();
  const contentRef = useCallback(
    (node: any) => {
      ...
      setInTransition((prev) => (prev === undefined ? false : true));
      if (blockTimeout.current) clearTimeout(blockTimeout.current);
      blockTimeout.current = setTimeout(() => setInTransition(false), 360);
